...
import { Order, Receipt, ReceiptRecord } from '../types';
import { Folder, FileSpreadsheet, Upload, Trash2, ChevronRight, Calendar, Plus } from 'lucide-react';
...
export function FileManager({ orders, onUpdateOrder, onDeleteOrder, onCreateOrder }: FileManagerProps) {
  const [selectedOrderId, setSelectedOrderId] = useState<string | null>(null);
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [updatingFileId, setUpdatingFileId] = useState<string | null>(null);
  const [uploadType, setUploadType] = useState<'order' | 'receipt' | null>(null);
  const [orderToDelete, setOrderToDelete] = useState<{id: string, name: string} | null>(null);
  const [fileToDelete, setFileToDelete] = useState<{type: 'order' | 'receipt', fileId: string, fileName: string} | null>(null);
  ...
  const confirmDeleteFile = async () => {
    ...
    if (type === 'order') {
      const remainingFiles = selectedOrder.orderFiles?.filter(f => f.id !== fileId) || [];
      updatedOrder.orderFiles = remainingFiles;
    } else {
      const remainingReceipts = selectedOrder.receipts?.filter(r => r.id !== fileId) || [];
    ...
    reader.onload = async (evt) => {
      try {
        ...
        if (uploadType === 'order') {
          ...
          if (updatingFileId) {
            updatedFiles = updatedFiles.map(f => f.id === updatingFileId ? { ...f, fileName: file.name, importedAt: new Date().toISOString(), records } : f);
          } else {
            updatedFiles = [...updatedFiles, { id: crypto.randomUUID(), fileName: file.name, importedAt: new Date().toISOString(), records }];
          }
          updatedOrder.orderFiles = updatedFiles;
        } else {
          let updatedReceipts = selectedOrder.receipts || [];
          if (updatingFileId) {
            updatedReceipts = updatedReceipts.map(r => r.id === updatingFileId ? { ...r, fileName: file.name, importedAt: new Date().toISOString(), records } : r);
          } else {
            updatedReceipts = [...updatedReceipts, { id: crypto.randomUUID(), fileName: file.name, importedAt: new Date().toISOString(), records }];
          }
        ...
        if (records.length === 0) {
          alert("Cảnh báo: Không tìm thấy dữ liệu sản phẩm nào trong file. Vui lòng kiểm tra lại định dạng file.");
        } else {
          const matchedCount = records.filter(r => r.itemId && !r.itemId.includes('-')).length; // Simple check if it matched an existing item
          if (uploadType === 'receipt' && matchedCount === 0) {
            alert(`Đã tải lên file thành công, nhưng không tìm thấy sản phẩm nào khớp với đơn hàng gốc trong ${records.length} dòng dữ liệu.`);
          } else {
            alert(updatingFileId ? 'Đã cập nhật file thành công!' : 'Đã tải lên file mới thành công!');
          }
        }
      } catch (err) {
        console.error("Error processing file:", err);
        alert("Có lỗi khi xử lý file Excel hoặc lưu dữ liệu. Vui lòng thử lại.");
      } finally {
        setIsMapping(false);
      }
    };
      ...
      <div className="w-1/3 bg-white rounded-2xl shadow-sm border border-slate-200 flex flex-col overflow-hidden">
        <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
          ...
          <button 
            ...
            title="Thêm danh mục (Đơn hàng) mới"
          >
          ...
          {orders.map(order => (
            ...
            >
              <div className="flex items-center gap-3 truncate">
                <Folder className={`w-5 h-5 shrink-0 ${selectedOrderId === order.id ? 'text-emerald-500' : 'text-slate-400'}`} />
              ...
              <div className="flex items-center gap-2">
                ...
                <ChevronRight className={`w-4 h-4 shrink-0 ${selectedOrderId === order.id ? 'text-emerald-500' : 'text-slate-300'}`} />
              </div>
            </div>
          ))}
          {orders.length === 0 && (
              ...
              <div>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 text-sm font-medium text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg flex items-center gap-1.5 transition-colors"
                  >
                    <Upload className="w-4 h-4" />
                    Thêm File Đặt Hàng
                  </button>
                </div>
                {(!selectedOrder.orderFiles || selectedOrder.orderFiles.length === 0) ? (
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    {selectedOrder.orderFiles.map(file => (
                      <div key={file.id} className="flex items-center justify-between p-4 rounded-xl border border-slate-200 hover:border-blue-200 hover:shadow-md transition-all group">
                        <div className="flex items-center gap-4">
                          <div className="w-10 h-10 rounded-lg bg-blue-100 flex items-center justify-center text-blue-600 shrink-0">
                            <FileSpreadsheet className="w-5 h-5" />
                          </div>
                          <div>
                            <h4 className="font-medium text-slate-900">{file.fileName}</h4>
                            <div className="flex items-center gap-3 text-xs text-slate-500 mt-1">
                              <span className="flex items-center gap-1">
                                <Calendar className="w-3 h-3" />
                                {new Date(file.importedAt).toLocaleString('vi-VN')}
                              </span>
                              <span>•</span>
                        ...
                        <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button
                            onClick={() => handleUpdateFileClick('order', file.id)}
                            className="px-3 py-1.5 text-sm font-medium text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg flex items-center gap-1.5 transition-colors"
                          >
                            <Upload className="w-4 h-4" />
                            Upload lại
                          ...
                          <button
                            onClick={() => {
                              const f = selectedOrder.orderFiles?.find(f => f.id === file.id);
                              setFileToDelete({ type: 'order', fileId: file.id, fileName: f?.fileName || 'file' });
                            }}
              ...
              <div>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 text-sm font-medium text-emerald-600 bg-emerald-50 hover:bg-emerald-100 rounded-lg flex items-center gap-1.5 transition-colors"
                  >
                    <Upload className="w-4 h-4" />
                    Thêm File Hàng Về
                  </button>
                ...
                {(!selectedOrder.receipts || selectedOrder.receipts.length === 0) ? (
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    {selectedOrder.receipts.map(receipt => (
                      <div key={receipt.id} className="flex items-center justify-between p-4 rounded-xl border border-slate-200 hover:border-emerald-200 hover:shadow-md transition-all group">
                        <div className="flex items-center gap-4">
                          <div className="w-10 h-10 rounded-lg bg-emerald-100 flex items-center justify-center text-emerald-600 shrink-0">
                            <FileSpreadsheet className="w-5 h-5" />
                          </div>
                          <div>
                            <h4 className="font-medium text-slate-900">{receipt.fileName}</h4>
                            <div className="flex items-center gap-3 text-xs text-slate-500 mt-1">
                              <span className="flex items-center gap-1">
                                <Calendar className="w-3 h-3" />
                                {new Date(receipt.importedAt).toLocaleString('vi-VN')}
                              </span>
                              <span>•</span>
                        ...
                        <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button
                            onClick={() => handleUpdateFileClick('receipt', receipt.id)}
                            className="px-3 py-1.5 text-sm font-medium text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg flex items-center gap-1.5 transition-colors"
                          >
                            <Upload className="w-4 h-4" />
                            Upload lại
                          ...
                          <button
                            onClick={() => {
                              const r = selectedOrder.receipts?.find(r => r.id === receipt.id);
                              setFileToDelete({ type: 'receipt', fileId: receipt.id, fileName: r?.fileName || 'file' });
                            }}
                            className="p-1.5 text-rose-500 hover:bg-rose-50 rounded-lg transition-colors"
                            title="Xóa file"
                          >
